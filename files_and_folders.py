...
#        print("File " + str(file) + "exists.\n")
        ...
        text = "There is no file " + str(file) + ". The CT-HYB needs Delta_tau.dat to run DMFT."
        print(text)
...
def check_lambda_file_exist(lambda_file_name):
    if (os.path.exists(lambda_file_name)):
        ...
        text = "There is no file " + str(lambda_file_name) + ". The CT-HYB needs Delta_tau.dat to run DMFT."
        print(text)
        log_file = open("log.file", "a")
        log_file.write(text)
        log_file.close()
...
def check_ktau_file_exists(file):
    if (os.path.exists(file)):                                                                  #
        ...
        text = "There is no file " + str(file) + \
